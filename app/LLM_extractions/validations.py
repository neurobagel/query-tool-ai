...
def validate_age_order(filtered_ordered_response: dict) -> Optional[Any]:
    """
    ...
    """
    if "min_age" in filtered_ordered_response and "max_age" in filtered_ordered_response:
        min_age = int(filtered_ordered_response["min_age"])
        max_age = int(filtered_ordered_response["max_age"])
        
        if min_age > max_age:
            confirmation = input(f"Sorry but minimum age ({min_age}) can’t be bigger than maximum age ({max_age}). Would you like to see the result for min age {max_age} and max age {min_age}? If yes, please confirm (yes/no): ").lower()
            ...
            if confirmation == "yes":
                ...
                return "Sorry but minimum age can’t be bigger than maximum age."

    return None


def validate_diagnosis_and_control(filtered_ordered_response: dict) -> Optional[str]:
    """
    ...
    if "diagnosis" in filtered_ordered_response:
        if "is_control" in filtered_ordered_response and filtered_ordered_response["is_control"]:
